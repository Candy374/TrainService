...
namespace CommonUtilities
{
    ...
    public static class JunctionHelper
    {
        ...
        public static void MirrorFilesAsJunction(string sourceDir, string destDir, IEnumerable<string> copyFileWhichHasExt, bool overwrite = false)
        {
            if (!Directory.Exists(sourceDir))
            {
                throw new DirectoryNotFoundException("Could not found " + sourceDir);
            }

            if (!Directory.Exists(destDir))
            {
                IOHelper.CreateDir(destDir);
            }

            var extList = new List<string>();
            foreach (var ext in copyFileWhichHasExt)
            {
            ...
            foreach (var file in files)
            {
                try
                {
                    ...
                    if (extList.Contains(file.Extension.ToUpper()))
                    {
                        IOHelper.Copy(file.FullName, destFilePath);
                    }
                    else
                    {
                        CreateSymbolicLink(destFilePath, file.FullName, SymbolicLink.File);
                    }
                }
                catch (Exception e)
                {
            ...
            {
                MirrorFilesAsJunction(dir.FullName, Path.Combine(destDir, dir.Name), copyFileWhichHasExt, overwrite);
            }
        }
        ...
        public static void CreateDirJunction(string junctionPoint, string targetDir, bool overwrite)
        {
